...
[[ $version =~ ^(0|[1-9][0-9]{0,3})\.(0|[1-9][0-9]{0,2})\.(0|[1-9][0-9]{0,2})$ ]] || {
...
# Bash slices characters, not bytes, under the UTF-8 locale used by CI.
# Leave room for the trailing newline; do not use head in a pipefail pipeline.
changelog_body=${changelog_body:0:499}
